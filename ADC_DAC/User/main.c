...
int main(void)
{
    ...
    voltage_pid.target = 48.0f; //设置目标电压 48V
    ...
    while (1)
    {
        ...
        if (g_adc_dma_sta == 1) // 判断标志位g_adc_dma_sta && g_adc2_dma_sta == 1
        {
            ...
            adc2_value = adc2_get_result_average(ADC2_ADCX_CHY, 1);
            ...
            else if (Modbus_Process(rxBuffer, sizeof(rxBuffer), txBuffer, &txLen) == MODBUS_OK) // 判断发送数据是否正常
            {
