...
void adc_init(void)
{
    ...
    g_adc2_handle.Instance = ADC2_ADCX;
    g_adc2_handle.Init.ClockPrescaler = ADC_CLOCKPRESCALER_PCLK_DIV4;        /* 4分频，ADCCLK = PCLK2/4 = 84/4 = 21Mhz */
    g_adc2_handle.Init.Resolution = ADC_RESOLUTION_12B;                      /* 12位模式 */
    g_adc2_handle.Init.DataAlign = ADC_DATAALIGN_RIGHT;                      /* 右对齐 */
    g_adc2_handle.Init.ScanConvMode = DISABLE;                               /* 非扫描模式 */
    g_adc2_handle.Init.ContinuousConvMode = DISABLE;                         /* 关闭连续转换 */
    g_adc2_handle.Init.NbrOfConversion = 1;                                  /* 1个转换在规则序列中 也就是只转换规则序列1 */
    g_adc2_handle.Init.DiscontinuousConvMode = DISABLE;                      /* 禁止不连续采样模式 */
...
void HAL_ADC_MspInit(ADC_HandleTypeDef *hadc)
{
    ...
    if(hadc->Instance == ADC2_ADCX)
    {
        GPIO_InitTypeDef gpio_init_struct;
        ADC2_ADCX_CHY_CLK_ENABLE();      /* 使能ADCx时钟 */
        ADC2_ADCX_CHY_GPIO_CLK_ENABLE(); /* 开启GPIO时钟 */

        /* AD采集引脚模式设置,模拟输入 */
        gpio_init_struct.Pin = ADC2_ADCX_CHY_GPIO_PIN;
        gpio_init_struct.Mode = GPIO_MODE_ANALOG;
        gpio_init_struct.Pull = GPIO_NOPULL;
        HAL_GPIO_Init(ADC2_ADCX_CHY_GPIO_PORT, &gpio_init_struct);
    }
}
...
uint32_t adc2_get_result(uint32_t ch)
{
    adc_channel_set(&g_adc2_handle, ch, 1, ADC_SAMPLETIME_480CYCLES);   /* 设置通道，序列和采样时间 */
    HAL_ADC_Start(&g_adc2_handle);                                       /* 开启ADC */
    HAL_ADC_PollForConversion(&g_adc2_handle, 10);                       /* 轮询转换 */

    return (uint16_t)HAL_ADC_GetValue(&g_adc2_handle);                   /* 返回最近一次ADC1规则组的转换结果 */
}
...
uint32_t adc2_get_result_average(uint32_t ch, uint8_t times)
{
    uint32_t temp_val = 0;
    uint8_t t;
